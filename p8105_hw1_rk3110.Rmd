...
Now we can take the mean of our factor variable in addition to our numeric and logical variables. We still cannot take the mean of the character variable as it cannot be converted to a numeric variable.
...
The variables in this data set are `r names(penguins)`. The data set is `r nrow(penguins)` rows long and `r ncol(penguins)` columns wide. The mean flipper length of the data set is `r mean(pull(penguins, flipper_length_mm), na.rm = TRUE)`mm.
